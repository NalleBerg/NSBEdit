...
A lightweight, standalone RTF notepad and programming editor for Windows. **v2026.06.23.10**
...
- **Portable and installed modes** — the shared database lives in `%APPDATA%\NSBEdit\nsbedit.db` and is preferred by both installed and portable launches whenever it exists. If `install.bat` is run from a folder that already has `nsbedit.db`, it is imported into AppData once; if the portable copy exists but AppData does not yet, the app now migrates it upward so both launch paths converge on the same settings and FTP profiles. When Ollama is available during install, the installer also pulls the default AI model (`qwen3-coder`) and the fallback model (`qwen2.5-coder:3b`) so the AI window can answer right away. Installed AI prefs now self-heal on startup if they still contain old `*-instruct` tags, the AI button no longer crashes on open, and the AI log/input panes wrap text instead of using a sideways scrollbar. The AI reply pane now uses the split prose/code renderer with the answer host scrollbar hidden, the status line shows the live clock on the right, and fenced code blocks keep their Scintilla sideways scrollbar; the remaining follow-up is making replies that start with code choose the right first visible block every time. The curl runtime is shipped inside the app folder and closing the AI window restores the main editor to the front without leaving it pinned topmost. The markdown renderer work is now based on cmark-gfm, with prose and code blocks split in the answer path; the last step is making the visible answer start on the right block without reintroducing a host scrollbar. The AI model menu still supports local offline model choices, and the cloud choices stay grayed out until sign-in. If Cloud is chosen before the user is signed in, Send opens the Ollama sign-in / sign-up page instead of pretending to send. The Cloud menu also includes **Check local models**, which opens a modal in-app progress dialog that checks installed Ollama models and keeps the Close button disabled until the run is finished. If neither writable location is available the app falls back to an in-memory database and warns the user. No personal database is shipped in the ZIP
