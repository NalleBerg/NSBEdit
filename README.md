...
A lightweight, standalone RTF notepad and programming editor for Windows. **v2026.06.11.10**
...
- **Portable and installed modes** — installed builds keep their database in `%APPDATA%\NSBEdit\nsbedit.db`; if `install.bat` is run from a folder that already has `nsbedit.db`, it is imported into AppData once, but an existing AppData database is never overwritten. When Ollama is available during install, the installer also pulls the two default AI models (`qwen2.5-coder:7b` and `qwen2.5-coder:3b`) so the AI window can answer right away. Installed AI prefs now self-heal on startup if they still contain old `*-instruct` tags. Portable builds create `nsbedit.db` next to the exe on first run. If neither writable location is available the app falls back to an in-memory database and warns the user. No personal database is shipped in the ZIP
