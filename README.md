...
A lightweight, standalone RTF notepad and programming editor for Windows. **v2026.09.05.14**
...
- Credits dialog (About → Credits): Scintilla, Lexilla, GDI+, MinGW-W64, SQLite3, libcurl/libssh2, rtf2html, Ollama sections with links
...
- **Convert to plain text** (Convert → Plain text) — flattens the active RTF document to plain text: **images are skipped** (removed cleanly, with no leftover blank or placeholder character) while **tables are kept as tab-separated columns** (each cell followed by a `[Tab]`) so they still line up neatly in txt-mode
...
- **Edition 3** — About dialog shows Edition: 3 below the version number (translated in all 14 UI languages via `ABOUT_EDITION`)
- **Portable and installed modes** — the shared database lives in `%APPDATA%\NSBEdit\nsbedit.db` and is preferred by both installed and portable launches whenever it exists. If `install.bat` is run from a folder that already has `nsbedit.db`, it is imported into AppData once; if the portable copy exists but AppData does not yet, the app now migrates it upward so both launch paths converge on the same settings and FTP profiles. When Ollama is available during install, the installer also pulls the default AI model (`qwen3-coder`) and the fallback model (`qwen2.5-coder:3b`) so the AI window can answer right away. Installed AI prefs now self-heal on startup if they still contain old `*-instruct` tags, the AI button no longer crashes on open, and the AI log/input panes wrap text instead of using a sideways scrollbar. The AI reply pane now uses the split prose/code renderer and keeps the query, the prose before the first code block, and the first code block together in one page; the remaining step is to finish scroll polish so the live stream hides scrolling and the rendered scrollbar follows the content height cleanly. The curl runtime is shipped inside the app folder and closing the AI window restores the main editor to the front without leaving it pinned topmost. The markdown renderer work is now based on cmark-gfm, with prose and code blocks split in the answer path. The AI model menu supports local offline model choices under a **Local:** header and Ollama **Cloud** models (e.g. `qwen3-coder:480b-cloud`, `gpt-oss:120b-cloud`, `deepseek-v3.1:671b-cloud`) under a **Cloud** header. Cloud entries stay greyed out until sign-in (Cloud → Sign in to Ollama, plus `ollama signin` in a terminal so the local daemon can proxy cloud models). The AI window now detects the real sign-in state from the local Ollama daemon (`POST /api/me`) every time it opens, so signing out with `ollama signout` — even while NSBEdit is closed — is picked up correctly; when signed out it drops cloud mode and reverts to your last-used local model (or the first local model if none). For a browser-free option, **Cloud → Add API key…** lets you paste an ollama.com API key: cloud `-cloud` models are then sent straight to `https://ollama.com/api` with a bearer token (no `ollama signin` needed), and the key is stored **encrypted** (AES-256, DPAPI-wrapped) in the local AppData database — never in the repo or the shipped ZIP. Cloud mode streams real answers through the same path as local mode — the **Stop** button and **Please Wait** spinner work identically, cloud reasoning models (which stream a separate `thinking` field) no longer duplicate their output, and code blocks always render in a single fully-copyable cell. The Cloud menu also includes **Check local models**, which opens a modal in-app progress dialog that checks installed Ollama models and keeps the Close button disabled until the run is finished. If neither writable location is available the app falls back to an in-memory database and warns the user. No personal database is shipped in the ZIP
...
- **FTP → Close connection** — closes the active FTP connection from the FTP menu; when nothing is connected the item is grayed out, and the separate direct-disconnect entry was removed because this command already does the job
