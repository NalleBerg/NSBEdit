    ...
    while (lineStart <= text.size()) {
        ...
        if (!inCode && trimmed.rfind(L"```", 0) == 0) {
            ...
            if (codeLang == L"markdown" || codeLang == L"md") {
                ...
                AiCodeCopy_BeginBlock(hLog, labelStart, currentHeaderEnd, copyLabel);
                Ai_AppendRichRun(hLog, L"\r\n", nullptr);
            }
            ...
            currentCode += line;
            currentCode += L"\r\n";
            AiCodeCopy_AppendCodeLine(hLog, line + L"\r\n");
    ...
    UINT_PTR, DWORD_PTR dwRefData)
{
    ...
    case WM_SETCURSOR: {
        ...
        if (AiCodeCopy_IsOverLink(hwnd, pt)) {
...
static void Ai_StartSendWorker(HWND hwnd, std::wstring prompt, std::wstring model, std::wstring fallback)
{
    ...
    std::thread([work = std::move(work)]() mutable {
        ...
        try {
            ...
            result->ok = false;
            result->error = L"AI request failed: unexpected error.";
        }
...
static LRESULT CALLBACK Ai_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NOTIFY: {
        ...
        if (hdr && hdr->idFrom == IDC_AI_LOG && hdr->code == EN_LINK) {
            ...
            if (AiCodeCopy_HandleLink(hdr->hwndFrom, link)) {
    ...
    case WM_AI_SEND_COMPLETE: {
        try {
            ...
            Ai_AppendLog(hwnd, L"Ollama: response rendering failed unexpectedly.");
        }
        return 0;
    }
    case WM_MEASUREITEM: {
