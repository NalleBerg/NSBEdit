...
## v2026.09.06.10 (Reopen closed tab; double-click opens a tab not a new window; dark-editor fix; installer file associations) - 06.09.2026 10:35
- **New: Reopen closed tab**: right-click the tab bar (or a tab) → **Reopen closed tab** brings back the last tab you closed, restored from the database (saved files reopen with caret/scroll; unsaved/untitled tabs come back with their exact content). The **last five** closed tabs are remembered; the item greys out when there is nothing to reopen, and exit-time closing does not fill the list. Localised in all languages.
- **Fix: double-clicking a file no longer opens a second window (or wipes your session)**: when NSBEdit was already running **elevated** (the installer launches it that way) a normal-privilege double-click failed the single-instance check and started a duplicate instance — two instances then clobbered each other's saved session, losing open tabs + recent list. Detection now finds the running window (works across privilege levels) and the window explicitly accepts the file hand-off, so a double-clicked file opens as a **tab in the one running window**.
- **Fix: new/opened plain-text tab honours the dark editor**: with the dark-editor option on in a light UI, a freshly created or file-launched plain-text tab could show a white background; it now uses the dark editor background.
- **Installer: optional file-type registration**: the installer asks whether to register NSBEdit for its supported file types; on yes it points “Open with NSBEdit” at the **installed** exe and adds NSBEdit to the *Open with* list for all supported types, so a double-click can never launch a stray copy from a download/zip folder.

## v2026.09.06.09 (Create New Project: pick a licence — 19 full licences plus a custom option) - 06.09.2026 09:13
- **New: choose a licence when you create a project**: the New Project dialog has a **Licence** dropdown offering **19 common licences** — The Unlicense, MIT, Apache 2.0, GNU GPL v2.0/v3.0, GNU LGPL v2.1/v3.0, GNU AGPL v3.0, BSD 2-Clause, BSD 3-Clause, ISC, Mozilla Public License 2.0, Boost 1.0, European Union Public Licence 1.2, Creative Commons Zero / BY / BY-SA 4.0, Artistic 2.0 and WTFPL — plus **Other (custom)**. On Create, the **full text** of the chosen licence is written to the project's `LICENSE` file, with the year and project name filled in where the licence has a copyright line (MIT, BSD, ISC).
- **Choosing a licence is mandatory**: the dropdown starts on **Choose License** (no value); creating without picking one shows *“You must choose a license for your project.”* and returns to the dialog, so a project can never be created without a licence. **Other (custom)** writes a `LICENSE` containing *“Add your custom license here”* and opens it in a tab for editing.
- **Licences travel inside the exe**: the 19 canonical (SPDX) licence texts are embedded in NSBEdit and seeded into its database on first run, so it works from a portable copy with no external files.

## v2026.09.05.14 (Create New Project: scaffold a fresh project from the Project menu; custom tooltip fixes) - 05.09.2026 14:13
- **New: Create a new project from the Project menu**: **Project → Create new project…** (now the first item, with a separator below it) opens a dialog that scaffolds a brand-new project. Enter a **project name**, pick a **parent folder** (the browser’s *Make New Folder* lets you create your workspace folder on the spot), choose a **type** (CLI / Windows GUI / Database, default Windows GUI), and optionally edit the **build command** (default `makeit.bat`). The project is created in `<parent>\<project name>` — the full path fills in automatically and is shown in a live preview panel with the chosen type and build command.
- **What gets created**: the new folder gets a starter **README.md**, **CHANGELOG.md**, **.gitignore** and a **LICENSE** placeholder (UTF-8, no BOM). The project is registered, set active, and its **README.md** opens in a tab. Each project stores its **type**, **build command** and **run command** in the database (groundwork from 2026.09.05.11). A non-empty target folder is refused so nothing is overwritten.
- **Info panel with bold labels + full-path tooltip**: the summary shows **Full project path** on its own line (wraps so long paths are fully visible), then **Type** and **Build command**. Hovering the path shows the whole path in NSBEdit’s yellow tooltip, positioned **above the mouse pointer**.
- **Fix: the custom tooltip could stop working after a dialog closed**: the shared tooltip window, once first shown from a modal dialog, was destroyed with that dialog, leaving a stale handle so *no* tooltip appeared again anywhere until an app restart. It is now recreated whenever its handle is invalid, so tooltips keep working across every dialog. Also fixed a “shows only once” case where the tooltip popped up under the cursor and dismissed itself (it now anchors above the pointer).
- **i18n**: the new dialog/menu strings are English for now; other languages fall back to English until translated (keys grouped and clearly marked in the locale file).

## v2026.09.05.11 (Groundwork for Create New Project: per-project build settings in the database) - 05.09.2026 11:59
- **Foundation: projects now carry a type and build/run commands**: the projects table gains three new columns — `type`, `build_command` and `run_command` — laying the groundwork for the upcoming **Create New Project** feature (and the per-project build the AI needs later). The upgrade is **safe and automatic**: a fresh or portable database is created with the new columns, and an existing database made by an earlier build is upgraded in place on the next launch (guarded `ALTER TABLE ADD COLUMN`, so nothing is lost and it never runs twice). This also works on a **USB / truly-portable** copy — the schema and upgrade run at startup regardless of where the exe lives, so a stick used next to an installed copy carries the new columns from first creation. Any project whose root actually contains a `makeit.bat` is seeded with that as its build command; every other project stays unset.
- **Internal: project properties API**: a new read/write API (`NeProjectInfo` with get/set helpers) exposes the full project record — name, root, type, build command and run command — for the New Project dialog and future build actions. No visible change yet; the feature stays invisible until it is complete.

## v2026.09.03.09 (Bullet-proof crash guard: your open tabs always come back — across crash, reinstall, or file-launch; plus right-click “Copy path”) - 03.09.2026 09:07
- **Fix: Convert to plain text now skips images and keeps tables readable**: converting a rich document to plain text used to leave an inserted picture behind as a **stray blank/placeholder character**. Images are now **removed entirely** at the RTF level before the plain-text conversion (`\pict`, `\object`, and Word-drawing `\*\shppict`/`\*\nonshppict` groups are stripped, with any leftover object placeholder `U+FFFC` cleaned up), so the picture just disappears cleanly. **Tables are preserved** as tab-separated columns — each cell is followed by a **[Tab]** — so a table still lines up neatly in txt-mode. (HTML export is unaffected: it still **embeds** pictures as base64 data-URI `<img>` where you want them.)
- **New: right-click a tab → Copy path**: copies the tab’s full path to the clipboard. For an **FTP/SFTP** tab this is the **online remote path** (not the local temp file); for a local file it’s the on-disk path. The item only appears for tabs that actually have a path. (Localized in all 14 languages via `TAB_CTX_COPY_PATH`.)
- **Crash guard: opening a file no longer hides your session**: launching NSBEdit with a file (double-click / *Open with*) used to show only that file and silently stop autosaving, so a later crash/reinstall lost everything else. It now **always restores your full previous session first** (including unsaved tabs) and **merges** the opened file in — activating it if it’s already open. Your tabs are never replaced or hidden.
- **Crash guard: single instance**: only one NSBEdit owns the one session database. A second launch **forwards its file to the already-running window** (which adds it as a tab and comes to the front) instead of starting a rival instance that could overwrite the session.
- **Crash guard: debounced save-on-edit**: the session is now written to disk about a second after you stop typing (in both the RTF and code editors), on top of the existing 10-second timer, focus-loss save, and save-on-close. A crash now costs at most a fraction of a second of work.
- **Crash guard: two-slot rolling backup**: before each save the previous session snapshot is kept in a second slot. If the live slot is ever empty or bad, the previous-good session is restored automatically — belt and braces against any single bad write.
- **Crash guard: never drop a file on a failed read**: if a file can’t be read from disk during restore, the tab is kept with its last-known content from the database (marked unsaved) instead of being discarded. Only when there is nothing cached to fall back on is the tab reported and closed.
- **Fix: closing with several tabs open no longer trims your session to one**: the “Save changes?” prompt during close pumps messages, and a pending autosave timer could fire after the other tabs had already been torn down — writing a one-tab session over the full snapshot. Session autosave is now **frozen the moment the close sequence starts**, right after a final full snapshot, so the complete tab set is always what comes back. (Diagnosed from the DB: the full set had been preserved in the rolling-backup slot.)

## v2026.09.03.07 (AI can search the whole workspace; URLs open in the browser; all 14 UI languages complete; crash-guard now survives a reinstall; AI window zoom) - 03.09.2026 07:53
- **AI now treats the whole workspace as searchable text**: the project scan used an extension **allow-list**, so files without a listed extension — `app.manifest`, `.code-workspace`, dotfiles, and any extensionless file — were silently invisible to the AI. It now uses a binary **block-list** instead: every file counts as searchable text **except** known binary types (images, audio/video, archives, executables/objects, fonts, binary documents, databases, certificates, git packs), with a NUL-byte content guard to reject any binary that slips through with an unknown extension. New folders you add appear automatically (the tree is walked live on every request) and deleted ones drop off; **symlinked/junction folders are followed** (e.g. a `MyStyle` junction that travels between projects).
- **URLs open in the browser again**: typed or pasted web addresses in the editor are now auto-detected as clickable links (`EM_AUTOURLDETECT`). Hovering shows **NSBEdit's own localized tooltip** — the URL plus a *[Ctrl]+Click to open in browser* hint (translated in every UI language) — and **Ctrl+Click** opens the link in your default browser. Bare `www.` links get an `https://` scheme so they open correctly. Previously only manually inserted hyperlink fields were live; plain URLs did nothing.
- **All 14 interface languages are now 100% complete**: every shipped locale (English, Danish, German, Greek, Spanish, Finnish, French, Icelandic, Flemish, Dutch, Norwegian, Portuguese, Swedish and **Ukrainian**) now carries the full 566-key string set — verified by an automated key audit against the English reference. **Ukrainian**, which was only ~22% translated (and shipped a stub Regex Guide), has been fully translated, including the complete regex tutorial. A handful of newer keys missing from Swedish, Norwegian, Dutch, Greek, German and Danish were filled in, and the Spanish Ctrl+Click hint was translated.
- **Fix: unsaved edits survive a rebuild/reinstall**: the session autosave (the crash-guard that keeps a copy of every open tab) only wrote to the database on the 10-second timer or on a clean close. Rebuilding/reinstalling force-kills the running app (`taskkill /F`), which bypasses the clean-close save — so edits made in the last few seconds before a reinstall could be lost even though they should have been recoverable. NSBEdit now **also autosaves the moment it loses focus**, so switching away to rebuild captures your latest edits before the kill. (Verified end-to-end with a live crash test and a database dump: the pre-crash snapshot held all edits with the modified flag set, and the restart restored them.)
- **New: the AI window has its own zoom** (for tired eyes): **Ctrl +** / **Ctrl -** zoom in and out, **Ctrl 0** resets to 100%, and **Ctrl + mouse-wheel** zooms too — exactly like the editor. The **query box and the answer/conversation pane zoom together** so both stay the same size, and the chosen level is remembered across restarts.

## v2026.09.02.08 (Regex Guide: search jumps to the first match as you type — forward and backward; dead-key characters register; the guide no longer hides behind the app) - 02.09.2026 08:58
- **The Regex Guide search now jumps to the first match as you type**: typing in the guide's search box highlighted every match but never scrolled the view, so the first hit could sit far off-screen. The guide now scrolls to the **top-most match on every keystroke**. Because the first match is re-found from the top each time, the view moves **down** as you make the term more specific and back **up** as you delete characters — the jump follows the first instance in both directions. The scroll is done by explicit line arithmetic (`EM_EXLINEFROMCHAR` + `EM_LINESCROLL`) rather than `EM_SCROLLCARET`, which is unreliable on a read-only RichEdit while the search box holds the keyboard focus.
- **Dead-key characters now register in the search**: on Nordic/German layouts a character such as **`^`** is a *dead key* — it only produces a character after the next keystroke composes it. The search box relied solely on `EN_CHANGE`, so those composed characters could be missed. The box now also re-scans on `WM_CHAR` and `WM_KEYUP` after the edit processes each message, so whatever ends up left of the caret — including composed dead keys — registers and re-triggers the jump.
- **Fix: the Regex Guide window no longer disappears behind the app**: the guide was created as an **unowned** top-level window, while the tooltip shown when you hover the red *clear* button is a shared **topmost** window owned by the main application. Hovering the clear button could therefore drop the guide behind the main window — it looked like the guide had *closed* even though the app kept running (and it happened on the very first hover after a fresh install). The guide is now **owned by its parent window**, so it always stays above the app and can never be pushed behind it.

## v2026.08.26.08 (AI query box: browse history both ways with arrows + Page Up/Down, and the box is focused the moment the window is) - 26.08.2026 08:27
- **Query history now scrolls both ways**: the AI query box remembers your previous prompts and you could step **up** into older ones, but there was no way back **down**. Now the arrow keys cross query boundaries in both directions — **Up** on the first line of the box loads the previous (older) query, **Down** on the last line loads the next (newer) one, and going down past the newest restores the draft you were in the middle of writing. Within a multi-line query the arrows still just move the caret line by line; only crossing the top/bottom edge moves between queries. The caret is placed at the **bottom** when you enter an older query and at the **top** when you enter a newer one, so holding a key keeps moving the same way. This makes the intended workflow work: go up several queries, copy a piece of text, then come back down to your current draft and paste it.
- **New: Page Up / Page Down step through the history one query at a time**, regardless of where the caret sits in the current text — a quick way to jump between queries without arrowing through each line.
- **Fix: the query box is active as soon as the AI window has focus**: opening, reopening, Alt-Tabbing to, or clicking the AI window used to leave the caret nowhere — you had to click inside the query box before you could type. The query box now takes keyboard focus automatically whenever the window gains focus, so you can type or paste straight away.

## v2026.08.25.15 (AI window: fix cloud "prompt too long", the Send freeze, and the Send crash on pasted console output; unified plain-text copy/paste) - 25.08.2026 15:04
- **Fix: sending a simple message failed with "prompt is too long"**: with a project active, every AI request prepended a project context that listed **up to 32,000 files** — on a project holding a large vendored tree that reached ~800,000 characters, which Ollama Cloud now rejects outright (its models cap the context at 131,072 tokens; the cloud used to silently truncate, now it refuses). The project file listing is now bounded by **both** a 2,000-entry cap **and** a ~24,000-character budget (developer files are listed shallow-first, so vendored library files are what get dropped), keeping the whole context comfortably within the model window.
- **Fix: the AI window froze on every Send (local and cloud models)**: the project-context builder walks the **entire active-project tree on the UI thread** before each send. With a project that contains a dependency cache — e.g. a **`vcpkg`** tree of ~100,000 files — that walk took long enough to look like a permanent freeze. Dependency-manager caches (`vcpkg`, `buildtrees`, `installed`, `.conan`, `.cargo`) are now skipped just like `node_modules`/`vendor`/`packages`, and the walk is hard-capped at 20,000 files, so a send stays responsive on any project.
- **Fix: crash a few seconds after Send when the prompt contained the project's own folder path**: pasting console output that included a PowerShell prompt such as `PS C:\…\MyProject>` fed the AI the project's **exact root path**. Resolving that reference computed a relative sub-path with `substr(root_length + 1)`, which runs **past the end of the string** when the reference *is* the root — throwing `std::out_of_range`, which was uncaught and terminated the app. The offset is now bounds-checked (a reference equal to the root is treated as "no file"), so any pasted path is safe. This was the root cause of the reported "crashes ~2 s after Enter/Send" with pasted CLI text.
- **Robustness: pasting from a terminal can no longer destabilise the query box**: clipboard text is now normalised through a **UTF-8 round-trip** (repairing any ill-formed or unpaired UTF-16 units) and passed through a **whitelist** that keeps real readable text — including language letters such as `Ø Æ Å ø æ å` — while dropping control, format, zero-width, bidi, private-use and noncharacter code points; a non-terminated clipboard block can no longer be read past its end.
- **Change: the AI query box copies as plain text**: **Ctrl+C / Ctrl+X** and the right-click **Copy/Cut** used RichEdit's rich-text export while paste inserted plain text, so the same selection behaved differently depending on how it was copied. Copy and Cut now export **plain** text, so every copy/cut/paste combination is consistent.
- **Robustness: a large paste can't stall the project search**: the prompt's file/pattern token extraction is now capped (64 pattern tokens, 40 file references), so a big console paste can no longer trigger an oversized project-wide grep.
- **Robustness: a moved or deleted active project self-heals**: if the folder of the active AI project no longer exists, the selection is cleared automatically instead of repeatedly trying to walk a missing path.
