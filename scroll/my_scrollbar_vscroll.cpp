...
{
    if (isRichEdit && vert) {
        /* V axis: EM_SHOWSCROLLBAR(SB_VERT, FALSE) reliably hides the native
         * vertical bar while keeping the NC slot intact (client rect unchanged),
         * which is what we want — the custom V bar sits inside the client area
         * and visually covers the now-empty NC slot. */
        SendMessageW(hWnd, EM_SHOWSCROLLBAR, SB_VERT, FALSE);
    } else {
        /* H axis (all targets including RichEdit), and non-RichEdit V axis:
         * ShowScrollBar(FALSE) collapses the NC area so the bar slot disappears
         * entirely.  For RichEdit H this is essential: EM_SHOWSCROLLBAR(SB_HORZ)
         * hides the bar visually but does NOT collapse the NC slot, so the native
         * bar remains below the client area — outside the region covered by the
         * custom H bar sibling — and both bars are visible simultaneously.
         * ShowScrollBar(FALSE) removes the slot, eliminating the double-bar. */
        ShowScrollBar(hWnd, vert ? SB_VERT : SB_HORZ, FALSE);
    }
}
...
{
    if (isRichEdit && vert)
        SendMessageW(hWnd, EM_SHOWSCROLLBAR, SB_VERT, TRUE);
    else
        ShowScrollBar(hWnd, vert ? SB_VERT : SB_HORZ, TRUE);
}
