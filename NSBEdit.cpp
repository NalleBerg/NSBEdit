...
// Autosave is DB-only. The periodic session timer calls Ne_SessionSave to
// CONSERVE the live tab content to the database for crash / restart recovery.
// It deliberately does NOT write the document's real file and does NOT clear
// the modified flag: a document becomes "saved" (and the status line shows
// Saved) only through an explicit save — Ctrl+S / Save / Save As — which is the
// only path that writes the file on disk.
...
static void ShowNsbAboutDialog(HWND parent)
{
    ...
    AppendNsbRich(hEdit, Ls(L"ABOUT_EDITION_VALUE"), false, RGB(60,60,60), 11, true);
    AppendNsbRich(hEdit, L"\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\r\n\r\n", false, RGB(180,20,20), 8, true);
...
static LRESULT CALLBACK Ne_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_TIMER:
        if (wParam == NE_TIMER_SESSION) {
            // DB-only conservation for crash / restart recovery. Does NOT write
            // the real file or clear the modified flag — only an explicit save
